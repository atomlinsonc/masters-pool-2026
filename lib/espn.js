...
async function fetchMastersLeaderboard() {
  ...
  const players = (competition.competitors || []).map((comp) => {
    ...
    // score.displayValue is NOT score-to-par (it shows "E" for all active players).
    // The real score-to-par lives in statistics[].name === "scoreToPar".
    const scoreToParStat = (comp.statistics || []).find(
      (s) => s.name === 'scoreToPar'
    );
    const scoreDisplay = scoreToParStat?.displayValue ?? comp.score?.displayValue ?? '--';
    const scoreValue =
      typeof scoreToParStat?.value === 'number'
        ? scoreToParStat.value
        : typeof comp.score?.value === 'number'
          ? comp.score.value
          : null;
